...
<div class="max-w-5xl mx-auto bg-white rounded-lg shadow-md overflow-hidden my-8">
    ...
    <div class="bg-gray-800 text-white px-6 py-4">
        <div class="flex justify-between items-center">
            ...
            <div class="text-right">
                <h2 class="text-xl font-semibold">Sample Order</h2>
                <p class="text-sm">SO-{{ str_pad($sampleOrder->order_id, 5, '0', STR_PAD_LEFT) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-blue-50 px-6 py-3 border-b border-gray-200">
        <div class="flex items-center gap-4">
            <span class="font-semibold {{ in_array($sampleOrder->status, ['pending', 'accepted']) ? 'text-blue-600 animate-blink' : '' }}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
        <div>
            ...
            <p class="mb-1"><span class="font-medium">Name:</span> {{ $sampleOrder->customer->name ?? 'N/A' }}</p>
            <p class="mb-1"><span class="font-medium">Requested By:</span> {{ $sampleOrder->addedBy->name ?? 'System' }}</p>
        </div>
        <div>
            <h3 class="font-semibold text-lg mb-2">Order Details</h3>
            <p class="mb-1"><span class="font-medium">Requested Delivery Date:</span> 
                {{ $sampleOrder->wanted_delivery_date ?: 'Not specified' }}
            </p>
            <p class="mb-1"><span class="font-medium">Created:</span> 
                {{ $sampleOrder->created_at->format('M d, Y H:i') }}
            </p>
        </div>
    </div>
    ...
    <div class="bg-yellow-50 border-l-4 border-yellow-400 mx-6 mb-6 p-4">
        ...
        <p class="text-yellow-700 whitespace-pre-line">{{ $sampleOrder->special_notes }}</p>
    </div>
    @endif
    ...
    <div class="px-6 pb-6">
        ...
        <div class="item-row mb-6 border rounded-lg overflow-hidden">
            ...
            <!-- Variations -->
            @if($item->is_variation && $item->variations->count() > 0)
            <div class="divide-y">
        ...
        <div class="flex flex-col md:flex-row items-center justify-between">
            <div class="mb-4 md:mb-0">
                ...
                <div class="text-blue-600 text-sm break-all">
                    {{ url("/sample-orders/{$sampleOrder->order_id}/{$sampleOrder->random_code}") }}
                </div>
            </div>
            <div class="bg-white p-3 rounded border">
                <img src="https://api.qrserver.com/v1/create-qr-code/?data={{ urlencode(url("/sample-orders/{$sampleOrder->order_id}/{$sampleOrder->random_code}")) }}&size=150x150" 
                     alt="QR Code" 
                     class="w-24 h-24">
            </div>
        </div>
