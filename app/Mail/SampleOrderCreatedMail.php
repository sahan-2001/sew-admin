...
class SampleOrderCreatedMail extends Mailable
{
     ...
     */
    public function __construct(SampleOrder $order)
    {
        $this->order = $order->load([
            'customer',
            'items.variations', // eager-load items and variations
        ]);
        ...
        $this->qrCodeUrl = url('/sample-orders/' . $this->order->order_id . '/' . $this->order->random_code);
        ...
        $qrCodeFilename = 'qrcode_' . $this->order->order_id . '.svg';
    ...
    {
        return $this->subject('New Sample Order Confirmation')
                    ->view('emails.sample-orders.created')
                    ->with([
