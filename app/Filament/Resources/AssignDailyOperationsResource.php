                                        ...
                                        ->schema([
                                            DatePicker::make('operation_date')
                                                ...
                                                ->disabled(function ($livewire) {
                                                        return $livewire instanceof \Filament\Resources\Pages\EditRecord || !auth()->user()->can('select_next_operation_dates');
                                                    })
                                                ->dehydrated(),
                                            Repeater::make('operation_times')
                                                ->label('Operation Time Slots')
                                                ->columns(3)
                                                ->schema([
                                                    TimePicker::make('from')
                                                        ...
                                                        ->afterStateUpdated(function (callable $get, callable $set, $state, $context) {
                                                            ...
                                                            // Check with previous time slot
                                                            if ($index > 0) {
                                                                $previousTo = $get('../../' . ($index - 1) . '.to');
                                                                if ($previousTo && $state < $previousTo) {
                                                                    Notification::make()
                                                                        ->title('Invalid sequence')
                                                                        ->body("Start time must be after or equal to previous slot's end time ({$previousTo}).")
                                                                        ->danger()
                                                                        ->send();
                                                                    $set('from', null);
                                                                    return;
                                                            ...
                                                            if ($from && $to) {
                                                                ...
                                                                $set('duration', trim($durationText));
                                                            } else {
                                                                $set('duration', null);
                                                            }
                                                        }),
